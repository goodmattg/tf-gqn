...
def _reduce_packed_representation(enc_r_packed, model_params):
    ...
    # reshape encoding
    height, width = tf.shape(input=enc_r_packed)[1], tf.shape(input=enc_r_packed)[2]
    enc_r_unpacked = tf.reshape(
        enc_r_packed, shape=[-1, ctx_size, height, width, dim_c_enc]
    )
    # add scene representations per data tuple
    enc_r = tf.reduce_sum(input_tensor=enc_r_unpacked, axis=1)
    return enc_r
...
def _encode_context(encoder_fn, context_poses, context_frames, model_params):
    ...
    enc_r_packed, endpoints_psi = encoder_fn(
        context_frames_packed, context_poses_packed
    )
...
def gqn_vae(
  ...
  """
    raise NotImplementedError(
        "The GQN version with a VAE decoder is currently under development! Use at your own risk!"
    )
    with tf.compat.v1.variable_scope(scope):
        endpoints = {}
