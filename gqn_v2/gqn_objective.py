...
def gqn_draw_elbo(
  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    target_llh = tf.identity(
        input=-tf.reduce_sum(
            input_tensor=tf.reduce_mean(input_tensor=target_normal.log_prob(target_frame), axis=0)),
        name='target_llh')
    ...
    for mu_q_l, sigma_q_l, mu_pi_l, sigma_pi_l in zip(mu_q, sigma_q, mu_pi, sigma_pi):
      posterior_normal_l = tf.compat.v1.distributions.Normal(loc=mu_q_l, scale=sigma_q_l)
      prior_normal_l = tf.compat.v1.distributions.Normal(loc=mu_pi_l, scale=sigma_pi_l)
      kl_div_l = tf.compat.v1.distributions.kl_divergence(posterior_normal_l, prior_normal_l)
      # kl_div_l = tf.Print(
      #     input_=kl_div_l,
    ...
    kl_regularizer = tf.identity(
        input=tf.reduce_sum(
            input_tensor=tf.reduce_mean(input_tensor=tf.add_n(kl_div_list), axis=0)),
        name='kl_regularizer')
...
def gqn_vae_elbo(
  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    posterior_normal = tf.compat.v1.distributions.Normal(loc=mu_q, scale=sigma_q)
    prior_normal = tf.compat.v1.distributions.Normal(loc=tf.zeros_like(mu_q),
                                          scale=tf.ones_like(sigma_q))
    kl_div = tf.compat.v1.distributions.kl_divergence(posterior_normal, prior_normal)
    kl_regularizer = tf.identity(
        input=tf.reduce_sum(
            input_tensor=tf.reduce_mean(input_tensor=tf.add_n(kl_div), axis=0)),
        name='kl_regularizer')
