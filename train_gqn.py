...
from gqn_v2.gqn_model import gqn_draw_model_fn
from gqn_v2.gqn_params import create_gqn_config
...
def main(unparsed_argv):
  ...
  """
    ...
    gpu_options = tf.compat.v1.GPUOptions(
        allow_growth=True, per_process_gpu_memory_fraction=ARGS.memcap
    )
    sess_config = tf.compat.v1.ConfigProto(gpu_options=gpu_options)
    run_config = tf.estimator.RunConfig(
        session_config=sess_config, save_checkpoints_steps=ARGS.chkpt_steps
    )
    ...
    input_fn = lambda estimator_mode: EagerDataReader(
...
if __name__ == "__main__":
    ...
    tf.compat.v1.app.run(argv=[sys.argv[0]] + UNPARSED_ARGS)
